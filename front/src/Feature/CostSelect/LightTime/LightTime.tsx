import React from 'react';
import LinkBox from '../../../Component/NavBox/NavBox';
import RadioBox from '../../../Component/RadioBox/RadioBox';
import SelectFrame from '../../../Component/SelectFrame/SelectFrame';
import {
  CoffeType,
  LightTimeType,
  MeetType,
} from '../../../Constant/DailyType';
import { PaginationInfo } from '../../../Constant/Page';
import useDailySelect from '../../../hook/useDailySelect';
import useCarSelect from '../CarSelect/useCarSelect';

function LightTimeSelect() {
  const { setSelectedItem } = useDailySelect();

  return (
    <SelectFrame
      title="오늘 전등을 얼마나 켜두셨나요?"
      pageNum={PaginationInfo.LightTime}
    >
      <RadioBox choices={LightTimeType} setStorage={setSelectedItem}></RadioBox>
      <LinkBox prevLink="/select/computertime" nextLink="/select/isDriving" />
    </SelectFrame>
  );
}

export default LightTimeSelect;
