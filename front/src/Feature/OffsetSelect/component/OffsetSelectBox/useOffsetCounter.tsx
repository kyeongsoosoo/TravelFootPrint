...
export function useOffsetCount() {
  ...
  const OffsetQuestion = OffsetType.filter((item: OffsetSelectType) =>
    DailyValueList.includes(item.category)
  );

  if (DrivingList.isDriving) {
    OffsetQuestion.push(
      OffsetType.find((item) => item.category === "자동차 타기") ||
        OffsetType[0]
    );
  }

  const DrvingCheckedQuestion = DrivingList.total
    ? OffsetQuestion.map((item) => {
        if (item.category === "자동차 타기")
          item.cost = Math.floor(parseInt(DrivingList.total || "0", 10) * 0.12);
        return item;
      })
    : OffsetQuestion;
